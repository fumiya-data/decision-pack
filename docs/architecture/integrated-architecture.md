...
- 複数の入力ソースを扱う場合は、ソースごとの adapter を用意する
- adapter の出力は共通 staging へ集約し、その後に正規化テーブルへ流す
- ソースごとに最終テーブルへ直接書き込む方式は採らない

## 6. PostgreSQL の最小論理設計

PostgreSQL の schema 変更は、生 SQL の migration ファイルを `db/migrations/` で管理し、migration runner で適用します。
...
重みは固定値で埋め込まず、検証データを使って調整する前提で管理します。
...
基本導線は `顧客`, `在庫`, `シミュレーション` の並列タブとします。
...
- 品目ごとの在庫一覧
- 在庫ステータスの絞り込み検索
- 品目カテゴリ別閾値による欠品リスク、過剰在庫リスクの表示
- MOQ と lot size を考慮した推奨補充量の表示
...
3. 商品遷移確率の初版実装
4. 検証データを使った重み調整
5. `customer_item_next_buy_score` 生成ジョブ
6. `item_demand_forecast` 集約ジョブ
